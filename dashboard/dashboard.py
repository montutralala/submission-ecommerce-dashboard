"""
Dashboard Analisis E-Commerce Public Dataset
Dibuat menggunakan Streamlit
"""

import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt
import matplotlib.ticker as mticker
import seaborn as sns
import numpy as np
import os

# ─────────────────────────────────────────────────────────────────────────────
# KONFIGURASI HALAMAN
# ─────────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="E-Commerce Dashboard",
    page_icon="🛒",
    layout="wide",
    initial_sidebar_state="expanded"
)

sns.set_theme(style="whitegrid", palette="muted")
plt.rcParams.update({
    "figure.dpi": 120,
    "axes.titlesize": 13,
    "axes.titleweight": "bold",
    "axes.labelsize": 10,
    "xtick.labelsize": 8,
    "ytick.labelsize": 8,
})


# ─────────────────────────────────────────────────────────────────────────────
# LOAD DATA
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_data
def load_data():
    base = os.path.dirname(__file__)

    main_df = pd.read_csv(os.path.join(base, "main_data.csv"),
                          parse_dates=["order_purchase_timestamp"])

    rfm_df = pd.read_csv(os.path.join(base, "rfm_data.csv"))

    monthly_revenue = pd.read_csv(os.path.join(base, "monthly_revenue.csv"))

    cat_perf = pd.read_csv(os.path.join(base, "category_performance.csv"))

    return main_df, rfm_df, monthly_revenue, cat_perf


main_df, rfm_df, monthly_revenue, cat_perf = load_data()

# ─────────────────────────────────────────────────────────────────────────────
# SIDEBAR
# ─────────────────────────────────────────────────────────────────────────────
with st.sidebar:
    st.title("🛒 E-Commerce Dashboard")
    st.markdown("**Brazilian E-Commerce Public Dataset**")
    st.divider()

    st.subheader("📅 Filter Rentang Waktu")
    min_date = main_df["order_purchase_timestamp"].dt.date.min()
    max_date = main_df["order_purchase_timestamp"].dt.date.max()

    date_range = st.date_input(
        "Pilih rentang tanggal:",
        value=(min_date, max_date),
        min_value=min_date,
        max_value=max_date
    )

    if len(date_range) == 2:
        start_date, end_date = date_range
    else:
        start_date, end_date = min_date, max_date

    st.divider()
    st.subheader("🔢 Filter Top N Kategori")
    top_n = st.slider("Tampilkan top N kategori:", min_value=5, max_value=20, value=10, step=1)

    st.divider()
    st.markdown("**Dibuat oleh:** Nathan Alfa Shidqi")
    st.markdown("**Dataset:** [Brazilian E-Commerce](https://www.kaggle.com/datasets/olistbr/brazilian-ecommerce)")


# ─────────────────────────────────────────────────────────────────────────────
# FILTER DATA BERDASARKAN SIDEBAR
# ─────────────────────────────────────────────────────────────────────────────
filtered_df = main_df[
    (main_df["order_purchase_timestamp"].dt.date >= start_date) &
    (main_df["order_purchase_timestamp"].dt.date <= end_date)
].copy()

# ─────────────────────────────────────────────────────────────────────────────
# HEADER
# ─────────────────────────────────────────────────────────────────────────────
st.title("🛒 Brazilian E-Commerce Analytics Dashboard")
st.markdown(
    f"Menampilkan data dari **{start_date.strftime('%d %b %Y')}** hingga **{end_date.strftime('%d %b %Y')}**"
)
st.divider()

# ─────────────────────────────────────────────────────────────────────────────
# METRICS RINGKASAN
# ─────────────────────────────────────────────────────────────────────────────
total_revenue  = filtered_df["price"].sum()
total_orders   = filtered_df["order_id"].nunique()
total_customers = filtered_df["customer_unique_id"].nunique()
avg_review     = filtered_df["review_score"].mean()
avg_order_val  = total_revenue / total_orders if total_orders > 0 else 0

col1, col2, col3, col4, col5 = st.columns(5)
col1.metric("💰 Total Revenue",      f"R$ {total_revenue:,.0f}")
col2.metric("📦 Total Pesanan",      f"{total_orders:,}")
col3.metric("👥 Total Pelanggan",    f"{total_customers:,}")
col4.metric("⭐ Avg Review Score",   f"{avg_review:.2f} / 5")
col5.metric("🧾 Avg Order Value",    f"R$ {avg_order_val:,.2f}")

st.divider()

# ─────────────────────────────────────────────────────────────────────────────
# TAB NAVIGASI
# ─────────────────────────────────────────────────────────────────────────────
tab1, tab2, tab3 = st.tabs([
    "📈 Tren Revenue Bulanan",
    "📦 Performa Kategori Produk",
    "👥 RFM Segmentasi Pelanggan"
])


# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║  TAB 1 — TREN REVENUE BULANAN                                            ║
# ╚═══════════════════════════════════════════════════════════════════════════╝
with tab1:
    st.subheader("📈 Tren Revenue & Volume Pesanan Bulanan")
    st.markdown(
        """
        Visualisasi ini menjawab pertanyaan bisnis:  
        **"Bagaimana tren pertumbuhan total pendapatan bulanan selama 2017–2018?"**
        """
    )

    # Hitung ulang berdasarkan filter
    filtered_df["purchase_yearmonth"] = (
        filtered_df["order_purchase_timestamp"].dt.to_period("M").astype(str)
    )
    monthly_filt = (
        filtered_df
        .groupby("purchase_yearmonth", as_index=False)
        .agg(total_revenue=("price", "sum"), total_orders=("order_id", "nunique"))
        .sort_values("purchase_yearmonth")
    )

    fig, axes = plt.subplots(2, 1, figsize=(13, 8), sharex=True)
    fig.suptitle("Tren Revenue & Volume Pesanan Bulanan", fontsize=14, fontweight="bold")

    x = range(len(monthly_filt))

    # Revenue
    bars = axes[0].bar(x, monthly_filt["total_revenue"] / 1e6,
                       color="steelblue", edgecolor="white", linewidth=0.5)
    if len(monthly_filt) > 0:
        max_idx = monthly_filt["total_revenue"].idxmax()
        bars[max_idx].set_color("#e05c2a")
        axes[0].annotate(
            f'Puncak\nR${monthly_filt["total_revenue"].max()/1e6:.2f}M',
            xy=(max_idx, monthly_filt["total_revenue"].max()/1e6),
            xytext=(max_idx + 1.2, monthly_filt["total_revenue"].max()/1e6 * 0.9),
            arrowprops=dict(arrowstyle="->", color="#e05c2a"),
            color="#e05c2a", fontsize=8, fontweight="bold"
        )
    axes[0].set_ylabel("Total Revenue (Juta BRL)")
    axes[0].yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"R${v:.1f}M"))
    axes[0].set_title("Total Revenue Bulanan", fontsize=11)
    axes[0].grid(axis="y", alpha=0.3)

    # Volume
    axes[1].plot(x, monthly_filt["total_orders"], marker="o", color="darkorange",
                 linewidth=2, markersize=5, zorder=3)
    axes[1].fill_between(x, monthly_filt["total_orders"], alpha=0.15, color="darkorange")
    axes[1].set_ylabel("Jumlah Pesanan")
    axes[1].set_title("Volume Pesanan Bulanan", fontsize=11)
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(monthly_filt["purchase_yearmonth"], rotation=45, ha="right", fontsize=7)
    axes[1].yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"{int(v):,}"))
    axes[1].grid(axis="y", alpha=0.3)

    plt.tight_layout()
    st.pyplot(fig)
    plt.close()

    # Data table
    with st.expander("📋 Lihat Data Tabel"):
        display_monthly = monthly_filt.copy()
        display_monthly["total_revenue"] = display_monthly["total_revenue"].map(lambda x: f"R$ {x:,.2f}")
        display_monthly["total_orders"] = display_monthly["total_orders"].map(lambda x: f"{x:,}")
        display_monthly.columns = ["Bulan", "Total Revenue", "Jumlah Pesanan"]
        st.dataframe(display_monthly, use_container_width=True)

    st.info(
        "💡 **Insight:** Revenue tumbuh konsisten sepanjang 2017 dengan puncak di **November 2017** "
        "(bertepatan dengan Black Friday Brasil). Revenue mulai melambat di 2018 meski volume "
        "pesanan tetap tinggi, mengindikasikan penurunan nilai rata-rata transaksi."
    )


# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║  TAB 2 — PERFORMA KATEGORI PRODUK                                        ║
# ╚═══════════════════════════════════════════════════════════════════════════╝
with tab2:
    st.subheader("📦 Performa Kategori Produk")
    st.markdown(
        """
        Visualisasi ini menjawab pertanyaan bisnis:  
        **"Kategori produk mana yang menghasilkan revenue tertinggi dan memiliki volume terbanyak?"**
        """
    )

    cat_filt = (
        filtered_df
        .groupby("product_category_name_english", as_index=False)
        .agg(
            total_revenue=("price", "sum"),
            total_orders=("order_id", "count"),
            avg_review=("review_score", "mean")
        )
        .sort_values("total_revenue", ascending=False)
    )

    top_rev = cat_filt.head(top_n)
    top_vol = cat_filt.sort_values("total_orders", ascending=False).head(top_n)

    fig, axes = plt.subplots(1, 2, figsize=(16, max(5, top_n * 0.5 + 1)))
    fig.suptitle(f"Top {top_n} Kategori Produk — Revenue vs Volume", fontsize=13, fontweight="bold")

    pal_rev = ["#1a5276" if i == 0 else "#2980b9" for i in range(top_n)]
    pal_vol = ["#784212" if i == 0 else "#e67e22" for i in range(top_n)]

    # Revenue
    b1 = axes[0].barh(top_rev["product_category_name_english"][::-1],
                      top_rev["total_revenue"][::-1] / 1e6,
                      color=pal_rev[::-1], edgecolor="white")
    for bar, val in zip(b1, top_rev["total_revenue"][::-1]):
        axes[0].text(bar.get_width() + 0.01, bar.get_y() + bar.get_height()/2,
                     f"R${val/1e6:.2f}M", va="center", fontsize=7)
    axes[0].set_xlabel("Total Revenue (Juta BRL)")
    axes[0].set_title("Revenue Tertinggi", fontsize=11)
    axes[0].xaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"R${v:.1f}M"))
    axes[0].grid(axis="x", alpha=0.3)

    # Volume
    b2 = axes[1].barh(top_vol["product_category_name_english"][::-1],
                      top_vol["total_orders"][::-1],
                      color=pal_vol[::-1], edgecolor="white")
    for bar, val in zip(b2, top_vol["total_orders"][::-1]):
        axes[1].text(bar.get_width() + 10, bar.get_y() + bar.get_height()/2,
                     f"{int(val):,}", va="center", fontsize=7)
    axes[1].set_xlabel("Jumlah Pesanan")
    axes[1].set_title("Volume Penjualan Tertinggi", fontsize=11)
    axes[1].xaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"{int(v):,}"))
    axes[1].grid(axis="x", alpha=0.3)

    plt.tight_layout()
    st.pyplot(fig)
    plt.close()

    # Scatter: Review vs Revenue
    st.subheader("⭐ Korelasi Review Score vs Revenue per Kategori")
    fig2, ax2 = plt.subplots(figsize=(10, 5))
    sc = ax2.scatter(
        cat_filt["avg_review"],
        cat_filt["total_revenue"] / 1e6,
        s=cat_filt["total_orders"] / 20,
        c=cat_filt["total_orders"],
        cmap="YlOrRd", alpha=0.7, edgecolors="gray", linewidth=0.4
    )
    for _, row in cat_filt.head(5).iterrows():
        ax2.annotate(row["product_category_name_english"],
                     xy=(row["avg_review"], row["total_revenue"]/1e6),
                     xytext=(4, 4), textcoords="offset points", fontsize=7)
    cb = plt.colorbar(sc, ax=ax2)
    cb.set_label("Volume Penjualan", fontsize=9)
    ax2.set_xlabel("Rata-Rata Skor Ulasan (1–5)")
    ax2.set_ylabel("Total Revenue (Juta BRL)")
    ax2.set_title("Korelasi Review Score vs Revenue (ukuran bubble = volume)", fontsize=11)
    ax2.grid(alpha=0.3)
    plt.tight_layout()
    st.pyplot(fig2)
    plt.close()

    st.info(
        "💡 **Insight:** **health_beauty** unggul di revenue; **bed_bath_table** dan "
        "**sports_leisure** mendominasi volume. Tidak ada korelasi kuat antara revenue tinggi "
        "dan skor ulasan tinggi — beberapa kategori top revenue justif memiliki review < 4.0."
    )


# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║  TAB 3 — RFM SEGMENTASI PELANGGAN                                        ║
# ╚═══════════════════════════════════════════════════════════════════════════╝
with tab3:
    st.subheader("👥 RFM Segmentasi Pelanggan")
    st.markdown(
        """
        Analisis lanjutan menggunakan **RFM (Recency, Frequency, Monetary)** untuk  
        menjawab pertanyaan: **"Kelompok pelanggan mana yang paling bernilai bagi platform?"**

        | Segmen | Kriteria |
        |---|---|
        | 🏆 Champions | Baru beli, sering, pengeluaran besar |
        | 💙 Loyal Customers | Sering beli, pengeluaran konsisten |
        | 🌱 Potential Loyalists | Baru beli beberapa kali, potensial |
        | ⚠️ At Risk | Dulu aktif, sekarang jarang |
        | 😴 Lost Customers | Sudah lama tidak bertransaksi |
        """
    )

    segment_order = ["Champions", "Loyal Customers", "Potential Loyalists", "At Risk", "Lost Customers"]
    colors_map = {
        "Champions": "#1abc9c",
        "Loyal Customers": "#3498db",
        "Potential Loyalists": "#f39c12",
        "At Risk": "#e74c3c",
        "Lost Customers": "#95a5a6"
    }

    seg_summary = (
        rfm_df.groupby("segment", as_index=False)
        .agg(
            jumlah_pelanggan=("customer_unique_id", "count"),
            avg_recency=("recency", "mean"),
            avg_frequency=("frequency", "mean"),
            avg_monetary=("monetary", "mean"),
            total_revenue=("monetary", "sum")
        )
    )
    seg_summary = seg_summary[seg_summary["segment"].isin(segment_order)].copy()
    seg_summary["segment"] = pd.Categorical(seg_summary["segment"],
                                            categories=segment_order, ordered=True)
    seg_summary = seg_summary.sort_values("segment")

    # Metrics per segmen
    cols = st.columns(5)
    seg_colors_list = ["#1abc9c", "#3498db", "#f39c12", "#e74c3c", "#95a5a6"]
    for i, seg in enumerate(segment_order):
        row = seg_summary[seg_summary["segment"] == seg]
        if not row.empty:
            cols[i].metric(
                label=seg,
                value=f"{int(row['jumlah_pelanggan'].values[0]):,} pelanggan",
                delta=f"R${row['avg_monetary'].values[0]:,.0f} avg spend"
            )

    # Bar charts
    fig, axes = plt.subplots(1, 3, figsize=(16, 5))
    fig.suptitle("Perbandingan Segmen Pelanggan (RFM Analysis)", fontsize=13, fontweight="bold")

    bar_colors = [colors_map.get(s, "gray") for s in seg_summary["segment"]]

    axes[0].bar(seg_summary["segment"], seg_summary["jumlah_pelanggan"],
                color=bar_colors, edgecolor="white")
    axes[0].set_title("Jumlah Pelanggan per Segmen", fontsize=10)
    axes[0].set_ylabel("Jumlah Pelanggan")
    axes[0].set_xticklabels(seg_summary["segment"], rotation=15, ha="right", fontsize=7)
    axes[0].yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"{int(v):,}"))
    axes[0].grid(axis="y", alpha=0.3)

    axes[1].bar(seg_summary["segment"], seg_summary["total_revenue"] / 1e6,
                color=bar_colors, edgecolor="white")
    axes[1].set_title("Total Revenue per Segmen", fontsize=10)
    axes[1].set_ylabel("Total Revenue (Juta BRL)")
    axes[1].set_xticklabels(seg_summary["segment"], rotation=15, ha="right", fontsize=7)
    axes[1].yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"R${v:.1f}M"))
    axes[1].grid(axis="y", alpha=0.3)

    axes[2].bar(seg_summary["segment"], seg_summary["avg_monetary"],
                color=bar_colors, edgecolor="white")
    axes[2].set_title("Rata-Rata Pengeluaran per Pelanggan", fontsize=10)
    axes[2].set_ylabel("Avg Monetary (BRL)")
    axes[2].set_xticklabels(seg_summary["segment"], rotation=15, ha="right", fontsize=7)
    axes[2].yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"R${v:.0f}"))
    axes[2].grid(axis="y", alpha=0.3)

    plt.tight_layout()
    st.pyplot(fig)
    plt.close()

    # Pie chart
    col_pie, col_table = st.columns([1, 1])
    with col_pie:
        fig3, ax3 = plt.subplots(figsize=(6, 6))
        pie_data = seg_summary.set_index("segment")["jumlah_pelanggan"]
        pie_colors = [colors_map.get(s, "gray") for s in pie_data.index]
        wedges, texts, autotexts = ax3.pie(
            pie_data,
            labels=pie_data.index,
            colors=pie_colors,
            autopct=lambda p: f"{p:.1f}%",
            startangle=140,
            pctdistance=0.80,
            wedgeprops=dict(edgecolor="white", linewidth=2)
        )
        for t in autotexts:
            t.set_fontsize(8)
        ax3.set_title("Proporsi Segmen Pelanggan", fontsize=11, fontweight="bold")
        plt.tight_layout()
        st.pyplot(fig3)
        plt.close()

    with col_table:
        st.subheader("📊 Tabel Ringkasan RFM")
        display_seg = seg_summary.copy()
        display_seg["jumlah_pelanggan"] = display_seg["jumlah_pelanggan"].map(lambda x: f"{x:,}")
        display_seg["avg_recency"]      = display_seg["avg_recency"].map(lambda x: f"{x:.0f} hari")
        display_seg["avg_frequency"]    = display_seg["avg_frequency"].map(lambda x: f"{x:.2f}x")
        display_seg["avg_monetary"]     = display_seg["avg_monetary"].map(lambda x: f"R${x:,.2f}")
        display_seg["total_revenue"]    = display_seg["total_revenue"].map(lambda x: f"R${x:,.2f}")
        display_seg.columns = ["Segmen", "Jml Pelanggan", "Avg Recency",
                               "Avg Frequency", "Avg Monetary", "Total Revenue"]
        st.dataframe(display_seg, use_container_width=True, hide_index=True)

    st.info(
        "💡 **Insight:** Lebih dari 60% pelanggan berada di segmen *At Risk* / *Lost Customers* — "
        "tingkat retensi sangat rendah. Segmen *Champions* meski sedikit, berkontribusi signifikan "
        "terhadap total revenue (prinsip **Pareto 80/20**). Diperlukan program loyalitas dan "
        "reaktivasi yang agresif untuk meningkatkan customer lifetime value."
    )

# ─────────────────────────────────────────────────────────────────────────────
# FOOTER
# ─────────────────────────────────────────────────────────────────────────────
st.divider()
st.markdown(
    "<div style='text-align:center; color:gray; font-size:12px;'>"
    "Brazilian E-Commerce Analytics Dashboard · Dibuat dengan Streamlit · "
    "Dataset: Olist E-Commerce Public Dataset (2016–2018)"
    "</div>",
    unsafe_allow_html=True
)
